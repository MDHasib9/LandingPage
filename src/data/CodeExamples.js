...
import { codeGo } from "@codeGo/ai";

function App() {
  const [code, setCode] = useState("");

  const handleAICompletion = async () => {
    const suggestion = await codeGo.complete(code);
    setCode(suggestion);
  };
...
export default function Hero() {
  ...
  const handleAISuggestion = async () => {
    const suggestion = await codeGo.suggest("hero component");
    return suggestion;
  };
...
export default function Navbar() {
  ...
  const handleSearch = async () => {
    const results = await codeGo.search(searchQuery);
    return results;
  };
