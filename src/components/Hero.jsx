...
// Demo data for code tabs and floating feature cards
import { codeExamples, floatingCards } from "../data/CodeExamples";

/**
 * Hero
 *
 * Primary above-the-fold section for the landing page.
 * Focuses on visual impact, product messaging, and an interactive code demo.
 */
export default function Hero() {
  // Tracks mouse position to drive the dynamic background gradient
  const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 });

  // Controls which code example is currently visible
  const [activeTab, setActiveTab] = useState("App.jsx");

  /**
   * Listen to mouse movement to create a cursor-following background effect.
   * Cleanup is required to avoid leaking global event listeners.
   *
   * Note: Throttling may be appropriate if this becomes performance-sensitive.
   */
  useEffect(() => {
  ...
  // Feature card content tied to the active code tab
  const currentFloatingCard = floatingCards[activeTab];

  return (
    <section className="relative min-h-screen flex items-center justify-center pt-16 sm:pt-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
      {/* Cursor-driven radial gradient for subtle depth */}
      <div
      ...
      {/* Ambient background blobs for visual interest */}
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:grid lg:grid-cols-2 text-center lg:text-left gap-6 sm:gap-8 lg:gap-12 items-center relative">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3 sm:gap-4 mb-8 sm:mb-12 animate-in slide-in-from-bottom duration-700 delay-300">
              ...
              <button className="group w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-2.5 bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg font-semibold text-sm sm:text-base transition-all duration-300 hover:bg-white/10 flex items-center justify-center space-x-2">
          ...
          <div className="relative order-2 w-full">
            <div className="relative bg-white/5 backdrop-blur-xl rounded-xl sm:rounded-2xl p-3 sm:p-4 shadow-2xl border border-white/10">
              <div className="bg-gradient-to-br from-gray-900/20 to-gray-800/20 backdrop-blur-sm rounded-lg overflow-hidden h-[280px] sm:h-[350px] lg:h-[450px] border border-white/5">
                <div className="flex items-center justify-between px-3 sm:px-4 py-2 sm:py-3 bg-white/5 backdrop-blur-sm border-b border-white/10">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs sm:text-sm text-gray-300">CodeGo AI</span>
                  </div>
              ...
              {/* Contextual feature card synced to the active tab */}
              <div
